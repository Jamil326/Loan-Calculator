...
class Calculate{
    ...
    getInterest(){
     ...
     getEmiBreakDown(){
          ...
          let emi =Math.round(this.getEmi());
          let monthlyRate = parseFloat(this.rate/1200);
          let month =1;
          let totalInterest = this.getInterest();
          let totalInter = 0;
          
          
          
        while( balance>0 && totalInterest>=interest ) {
            ...
            totalInter +=interest;
               ...
               if (this.transactions.length < monthTenure) {
        console.log("🚨 Warning: Loan ended early! Check calculations.");
    }

    // ✅ Log the correct total interest and total amount paid
    ...
    console.log('interest after month', totalInter);

              this.transactions.push({ emi:emi,interest:interest,principal:principal,balance:Math.max(0,balance),month:month});
               month++;  
            if (balance<=0) {
               balance = 0;
               break; 
          ...
          return {transactions:this.transactions,finalInterest:totalInterest,amount:this.amount};
