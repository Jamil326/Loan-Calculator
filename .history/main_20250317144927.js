...
getEmiBreakDown() {
    ...
    let monthTenure =parseFloat(this.time * 12);
    let principal, interest;
    let emi =parseFloat( this.getEmi()); // Keep EMI unrounded
    ...
    while (balance > 0 && month <= monthTenure) {
        interest =parseFloat ( Math.round(balance * monthlyRate)); // 🔥 Don't round yet
        principal =parseFloat (Math.round( emi - interest);

        // 🔥 Last EMI Fix: Prevent Overpayment
        if (principal > balance) {
            principal = balance;
            emi = interest + principal; // Adjust EMI to exact remaining amount
        }

        balance -= parseFloat( Math.round (principal));
    ...
    // 🔥 Ensure Total Amounts Match ₹90,234
    console.log("✅ Total Months:", this.transactions.length);
    console.log("✅ Total Interest Paid:", (totalInterest)); // Should be ₹75,234
    console.log("✅ Total Payment:", (totalPrincipal + totalInterest)); // Should be ₹90,234

    return {
        transactions: this.transactions,
        finalInterest: (totalInterest),
        totalAmountPaid: (totalPrincipal + totalInterest)
    };
}
