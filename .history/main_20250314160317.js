...
const createChart = (datas,typeOfChart,labels,elementId) => {
    ...
    const options = {
      ...
      type: typeOfChart,
       data: {
            labels: labels,  // Corrected 'lables' to 'labels'
            datasets: [{
                label: 'Dataset',  // Optional: Replace with dynamic label if needed
                data: datas,
                borderColor: 'rgba(75, 192, 192, 1)',  // Example border color
                fill: false // Optional: for line chart, you can decide whether to fill under the line
            }]
        }
    }
...
let typed = document.getElementById('select-wrapper');
    typed.addEventListener('input', (e) => {
        ...
        if(e.target.value !=='Select type of chart'){
            ...
            createChart(datas,labels,elementId);
        }
             }
