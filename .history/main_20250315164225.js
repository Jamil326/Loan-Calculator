...
class Calculate{
    ...
    getInterest(){
     ...
     getEmiBreakDown(){
          let balance = parseInt(this.amount);
          let monthTenure =parseInt(this.time*12);
          ...
          for( let month =1 ; month <= monthTenure; month++ ) {
              ...
              this.transactions.push({ emi:emi,interest:Math.floor(interest),principal:Math.floor(principal),balance:Math.floor(balance)
              ,month:month});
...
let tableData = cal.getEmiBreakDown();
...
const renderTable = (data) => {
    ...
    data.forEach(({month,emi,principal,balance,interest}) => {
       const row = document.createElement('tr');
       row.innerHTML = `<td> RS ${month}</td>
                       <td>${emi}</td>
                       <td>${interest}</td>
                       <td>${principal}</td>
                       <td>${balance}</td>`;
