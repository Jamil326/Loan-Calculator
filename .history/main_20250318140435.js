    ...
    inputInput.addEventListener(eventType, () => {
        let inputValue = parseFloat (inputInput.value, 10) || 0;
    ...
    rangeSlider.addEventListener(eventType, () => {
        let rangeSliderValue = parseFloat (rangeSlider.value, 10) || 0;
...
getEmiBreakDown() {
    ...
    let year =parseInt(t.getFullYear()) ;
    ...
    while (balance > 0 && month <= monthTenure) {
       ...
       // }
        ...
        if(balance < principal){
            //balance =0 ;
             diff = -(balance-principal);
              r = diff/month;
             console.log('diff', diff,balance,principal,r);
