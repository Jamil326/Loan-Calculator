...
class Calculate{
  ...
  getEmiBreakDown() {
    ...
    let month = 1;
    let totalInterest = 0;
    this.transactions = []; // Reset transactions to avoid duplicate entries

    while (balance > 0 && month <= monthTenure) {
        interest = Math.round(balance * monthlyRate);
        principal = emi - interest;

        // Adjust last month to ensure balance reaches exactly ₹0
        ...
        this.transactions.push({
            ...
            interest: interest,
            principal: principal,
            balance: Math.max(0, balance) // Prevents negative balance
        });

        month++;
    }

    // 🔥 Ensure loan is repaid over 360 months
