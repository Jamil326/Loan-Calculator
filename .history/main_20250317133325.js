...
getEmiBreakDown() {
    ...
    let emi = Math.round(this.getEmi()); // EMI should remain fixed
    let monthlyRate = parseFloat(this.rate / 1200);
    let month = 1;
    let totalInterest = 0;
    this.transactions = []; // Reset transactions

    while (balance > 0 && month <= monthTenure) {
        interest = balance * monthlyRate; // 🔥 Fix: Remove early rounding
        principal = emi - interest;

        // 🔥 Fix: Ensure last month's payment clears the exact balance
        if (principal > balance) {
            principal = balance;
            emi = interest + principal; // Adjust EMI for the final month
        }

        balance -= principal;
        totalInterest += interest;

        // 🔥 Fix: Round values only before pushing (not before calculations)
        this.transactions.push({
            month: month,
            emi: Math.round(emi),
            interest: Math.round(interest),
            principal: Math.round(principal),
            balance: Math.max(0, Math.round(balance)) // Prevents negative balance
        });

        month++;
    }

    // 🔥 Fix: Ensure exactly 360 months
    while (this.transactions.length < monthTenure) {
        this.transactions.push({
            month: this.transactions.length + 1,
            emi: 0,
            interest: 0,
            principal: 0,
            balance: 0
    ...
    // ✅ Log correct final results
    console.log("✅ Total Months:", this.transactions.length);
    console.log("✅ Total Interest Paid:", Math.round(totalInterest)); // Should be ₹75,235
    console.log("✅ Total Payment:", Math.round(this.amount + totalInterest)); // Should be ₹90,235

    return {
        transactions: this.transactions,
        finalInterest: Math.round(totalInterest),
        totalAmountPaid: Math.round(this.amount + totalInterest)
    };
}
