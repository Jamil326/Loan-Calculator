...
getEmiBreakDown() {
    ...
    let expectedTotalInterest = Math.round(this.getTotalPaid() - this.amount);
    let finalInterest = Math.round(totalInterest);
    let difference = finalInterest - expectedTotalInterest;
if (difference !== 0 && this.transactions.length > 0) {
    let lastIndex = this.transactions.length - 1;

    // Ensure we don’t reduce interest below zero
    if (this.transactions[lastIndex].interest >= difference) {
        this.transactions[lastIndex].interest -= difference;
    } else {
        // If difference is too large, adjust both interest and principal
        let excess = difference - this.transactions[lastIndex].interest;
        this.transactions[lastIndex].interest = 0;
        this.transactions[lastIndex].principal -= excess;
    }

    finalInterest -= difference;
}


    console.log("✅ Total Months:", this.transactions.length);
    console.log("✅ Total Interest Paid:", finalInterest);
    console.log("✅ Total Payment:", this.amount + finalInterest);

    return {
        transactions: this.transactions,
        finalInterest: finalInterest,
        totalAmountPaid: this.amount + finalInterest
    };
}
...
emi.textContent = '₹'+' '+datass.emi.toFixed(2);
interest.textContent = '₹'+" "+datass.totalInterest.toFixed(2);
total.textContent = '₹'+' '+datass.totalpaid.toFixed(2);
