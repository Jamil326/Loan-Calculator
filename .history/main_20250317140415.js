...
getEmiBreakDown() {
    ...
    while (balance > 0 && month <= monthTenure) {
        interest = balance * monthlyRate; // No early rounding
        principal = emi - interest;

        // 🔥 Last EMI Fix: Ensure exact balance clearance
        if (principal > balance) {
            principal = balance;
            emi = interest + principal; // Adjust EMI to exact remaining amount
        }
    ...
    // 🔥 Dynamically calculate expected total interest
    let expectedTotalInterest = Math.round(this.getTotalPaid() - this.amount);
    let finalInterest = Math.round(totalInterest);
    let totalPaid = Math.round(totalPrincipal + totalInterest);

    // 🔥 If there's a small mismatch, adjust last month's interest
    ...
    console.log("✅ Total Payment:", totalPaid);
